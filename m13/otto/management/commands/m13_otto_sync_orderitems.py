...
def check_order(json_order):
    ...
    for oi in db_order.orderitem_set.all():
        ...
        if otto_fulfillment_status != oi.fulfillment_status:
            print(f"FIXING - UPDATE otto_orderitem SET fulfillment_status = '{otto_fulfillment_status}' WHERE id = '{oi.id}'")
    ...
    if number_of_ois_otto != number_of_ois_m13:
        print(f"HUCH : number_of_ois_otto: {number_of_ois_otto} VS number_of_ois_m13: {number_of_ois_m13}")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for order_number in check_again:
            print(order_number)
