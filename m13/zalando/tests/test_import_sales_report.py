...
def test_import_sales_report():
    ...
    _import_sales_report(
        "zalando/tests/data/PP_E_131000027080_SAL_MAY_2022_(M)Manufaktur1_DE.CSV",
        "2022/05",
    )
    ...
    assert sales_report_import.created == datetime(
        2013, 10, 13, 0, 0, tzinfo=timezone.utc
    )
    assert sales_report_import.modified == datetime(
        2013, 10, 13, 0, 0, tzinfo=timezone.utc
    )
    assert sales_report_import.month == 202205

    assert (
        SalesReport.objects.filter(import_reference=sales_report_import.id).count() == 9
    )
